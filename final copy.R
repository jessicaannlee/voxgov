...
#data_dem <- filter(df,party=="democrat")

#data_dem$description <- as.character(data_dem$description)
#data_dem$name <- as.character(data_dem$name)
#data_dem$party <- as.character(data_dem$party)
#data_dem$party <- NULL

#write.csv(data_dem,"data_dem.csv")

  #data_rep <- filter(df,party=="republican")

    #data_rep$description <- as.character(data_rep$description)
    #data_rep$name <- as.character(data_rep$name)
    #data_rep$party <- as.character(data_rep$party)
    #data_rep$party <- NULL

#write.csv(data_rep,"data_rep.csv")

data_small <- read.csv("output_small.csv")

df_small <- tbl_df(data_small)

library(igraph)

colnames(df_small) <- c("name","description","party")

gD <- graph.data.frame(df_small)
...
# data_small$Sponsor = data_small$name

F1 <- function(x) {data.frame(V4 = dsAll[which(V(gD)$name == as.character(x$Sponsor)), which(V(gD)$name == as.character(x$description))])}
dataSet.ext <- ddply(data_small, .variables=c("Sponsor", "description", "party"), function(x) data.frame(F1(x)))
...
E(gD)[as.character(dataSet.ext$Sponsor) %--% as.character(dataSet.ext$description)]$weight <- as.character(dataSet.ext$party)
E(gD)[as.character(dataSet.ext$Sponsor) %--% as.character(dataSet.ext$description)]$similarity <- as.numeric(dataSet.ext$V4)
...
approxVals <- approx(c(1, 30), n = length(unique(V(gD)$betweenness)))
...
write.gexf(nodes = nodes_df, edges = edges_df, nodesAtt = nodes_att, edgesWeight = E(gD)$weight, edgesAtt = edges_att, defaultedgetype = "directed", output = "directed.gexf")
# And without edge weights
write.gexf(nodes = nodes_df, edges = edges_df, nodesAtt = nodes_att, edgesAtt = edges_att, nodesVizAtt = nodes_att_viz, edgesVizAtt = edges_att_viz, defaultedgetype = "undirected", output = "undirected.gexf")
