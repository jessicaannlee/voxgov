...
setwd("/Users/Jessica/Documents/Data Vis/Final Project/1 Work in Progress")
...
dataSet <- read.csv("20121102-15.csv")
...
F1 <- function(x) {data.frame(V3 = dsAll[which(V(gD)$name == as.character(x$bill_id)), which(V(gD)$name == as.character(x$poli_id))])}
dataSet.ext <- ddply(data, .variables=c("bill_id", "poli_id"), function(x) data.frame(F1(x)))

gD <- set.edge.attribute(gD, "weight", index = E(gD), value = 0)
gD <- set.edge.attribute(gD, "similarity", index = E(gD), value = 0)

E(gD)[as.character(dataSet.ext$bill_id) %--% as.character(dataSet.ext$poli_id)]$weight <- as.numeric(dataSet.ext$V3)
# next command doesn't work; replacement has length zero (no "V3" in our original dataset)
# E(gD)[as.character(dataSet.ext$target) %--% as.character(dataSet.ext$source)]$similarity <- as.numeric(dataSet.ext$V4)
...
write.gexf(nodes=nodes_df,edges = edges_df, nodesAtt = nodes_att, edgesWeight = E(gD)$weight, edgesAtt = edges_att, nodesVizAtt = nodes_att_viz, edgesVizAtt = edges_att_viz, defaultedgetype = "undirected", output = "gov.gexf")

# And without edge weights

write.gexf(nodes = nodes_df, edges = edges_df, nodesAtt = nodes_att, edgesAtt = edges_att, nodesVizAtt = nodes_att_viz, edgesVizAtt = edges_att_viz, defaultedgetype = "undirected", output = "gov_no.gexf")
